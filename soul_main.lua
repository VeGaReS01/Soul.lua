...
local license = "PAID"
local version = "developer-beta"
local support = "https://discord.gg/wp7uEFdqsg"
...
print("Successfully loaded SOUL.LUA.")
print("Loaded soul.lua version: " .. version)
print("SOUL License: " .. license)
print("Never expires support invite: " .. support)

-- CONSOLE
--@endregion
----------------------------------
--@region KILLSAY
-- KILLSAY (Just soul.lua promotion)

local killsays = {
    [1] = " SOUL.LUA - PREMIUM LUA FOR AIMWARE ",
    [2] = "",
    [3] = "",
    [4] = "",
    [5] = "",
    ...
    [12] = "",
    [13] = "",
    [14] = " SOUL.LUA - PREMIUM LUA FOR AIMWARE ",
}
...
{
    --exploits
    double_fire = gui.Checkbox(anti_aim_rage, "double_fire", "Double Tap", false),
...
{
    buy_bot = gui.Checkbox(misc_events, "buy_bot", "Enable Auto buy", false),
    buy_bot_primary = gui.Combobox(misc_events, "buy_bot_primary", "Auto buy Primary", "Nothing", "Autosniper", "AWP", "Scout"),
    buy_bot_secondary = gui.Combobox(misc_events, "buy_bot_secondary", "Auto buy Secondary", "Nothing", "Heavy Pistol", "Elite", "Fast Pistol", "P250"),
    buy_bot_extra = gui.Multibox(misc_events, "Auto buy Extra")
}
...
local function scriptPreparing()
    ...
    if not preparation_finished then
        ...
        local text = (delta_time > preparation_time) and "LOADING FINISHED" or "LOADING SOUL.LUA"
...
local function drawWatermark()
    ...
    watermark_text = ("SOUL   " .. handler_variables.user_name .. " | " ..  math.floor(handler_variables.fps_per_second) .. 
    ...
    if visuals_user_info_ui.watermark:GetValue() then
        ...
        local bleeding_text_size = {draw.GetTextSize("MIVI ")}
...
local function getFileName()
	...
	return string.sub(script_name, 1, string.find(script_name, ".lua") - 1) .. "_data.txt"
end
...
--@region SOUL_SETTINGS
local soul_settings = gui.Tab(gui.Reference("Misc"), "soul_settings", "Soul Settings");

--local soul_optional = gui.Groupbox(soul_settings, "Optional", 5, 10, 325, 450);
...
local buttons_ui = 
{
    advanced_preset = gui.Button(anti_aim_presets_tab, "Advanced", function()
...
--@endregion 
----------------------------------
--@region SCREEN_INFORMATIONS
-- SCREEEN INFORMATIONS
...
callbacks.Register("Draw", function()
    ...
    if localPlayer ~= nil then
        ...
        isFakeDucking = cheat.IsFakeDucking()
        isThirdperson = gui.GetValue("esp.world.thirdperson")
        isScoped = localPlayer:GetProp("m_bIsScoped") == 1
        doubleTap = anti_aim_rage_ui.double_fire:GetValue()
    end

    draw.Color(0, 0, 0, 180)
    draw.FilledRect(50, 675, 300, 516)

    draw.SetFont(Font)
    draw.Color(0, 209, 255, 255)
    draw.Text(60, 525, "Soul.lua")
    draw.Text(60, 550, "Name: ")
    draw.Text(60, 575, "Scoped: ")
    draw.Text(60, 600, "Fakeduck: ")
    draw.Text(60, 625, "Thirdperson: ")
    draw.Text(60, 650, "Doubletap: ")

    draw.Color(255, 255, 255)
    draw.Text(124, 525, "(" .. version .. ")")
    draw.Text(112, 550, playerName)
    draw.Text(123, 575, string.upper(isScoped and "TRUE" or "FALSE"))
    draw.Text(139, 600, string.upper(tostring(isFakeDucking)))
    draw.Text(157, 625, string.upper(tostring(isThirdperson)))
    draw.Text(142, 650, "TRUE")

    draw.Color(0, 209, 255)
    draw.OutlinedRect(50, 675, 300, 516)
end)
